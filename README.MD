...
| Program unit types           | [Functions](#4-function) , [modules](#modules), [classes](#classes-and-oop) |
...
In Python 3.0 (non in 2.6) un `single starred name`, ad esempio `*x`, può essere utilizzato nell'assegnazione (e nella firma di una funzione) per specificare che il `name` è assegnato ad una lista, cosicché gli siano assegnati tutti i valori di una sequenza e non vengano assegmati ad altri `names`. 
...
| def, <br/> return | def adder(a, b=1, *c): <br/> return a + b + c[0]      |
| global            | def changer(): <br/> global x; x = 'new'              |
| nonlocal          | def changer(): <br/> nonlocal x; x = 'new'            |
| yield             | def squares(x): <br/> for i in range(x): yield i ** 2 | 
| lambda            | funcs = [lambda x: x**2, lambda x: x*3]               |

## Coding functions

Le funzioni sono definite tramite lo `statment` `def`.

- **`def` is executable code.** `def` è un `executable statment`, ciò significa che la `function` non esiste fintanto che non è ricercato ed esiguito `def`. Infatti, è legale comporre `def` statments in `if` statments e `while` loops. Pertanto, `def` statments è codificato in `module files` e viene eseguito per generare `functions` quando un `module file` è importato.
- **`def` creates an object and assigns it to a name.** Come nelle assegnazioni, il nome della funzione diventa un riferimento al `function object`.
- **`lambda` creates an object but returns it as a result.** `functions` possono essere create con `lambda` expression quando non è possibile definire `in-line` function (vedere (Advanced Function Topic)[#advanced-function-topic])
- **`return` sends a result object back to the caller**
- **`yield` sends a result object back to the caller, but remembers where it left
off.** Functions definite come `generators` usano lo statments `yield` per restituire un valore e bloccare il proprio stato, per poi essere riesumate successivamente
- **`global` declares module-level variables that are to be assigned.** Le variabili definite in una `function` sono locali, per riferirci ad una globale è necessario utilizzare lo statement `global`.
- **`nonlocal`** `nonlocal` statements è utilizzata per riferirsi ad un `name` già esistente nello `scope` del `def` statements superiore.
- **Arguments are passed by assignment (object reference).**
- **Arguments, return values, and variables are not declared.** Nulla a riguardo di una funzione è dichiarato anzitempo: no tipi negli argomenti, restituisce qualasi tipo di oggetto e così via. 

Esempio di applicazione di `nonlocal`:
```python
x = 0
def outer():
    x = 1
...
Esempio applicazione di `global`:
```python
x = 0
def outer():
    x = 1
...
`def` è un `executable statement`: quando viene eseguita crea un nuovo `function object` e gli assegna un nome. Dato che è uno `statement` può essere definito dovunque
```python
if test:
...
```
e sul oggetto restituito possono essere eseguite operazioni quali `=`
```python
othername = func
othername()
```
oppure aggiungere `attributes` (che formeranno un qualcosa di simile ad uno stato di un oggetto nella OOP) per registare informazioni utili successivamente
```python
...
Dato che Python è linguaggio dinamicamente tipato, ogni operazione è un'operazione polimorfica.
...
**NOTA: per gli `object` che non supportano `*` è lanciata un'eccezione.**

## Scopes
### Python Scope Basic
Quando è utilizzato un `name`, questo è contenuto in un `namespace`. Invece, quando è utilizzato `name's assignement`, il termine `scope` si riferisce ad un `namespace` determinato dalla visibilità del `name`. Esempio: nomi definiti dentro un `def` possono essere viste solo dentro il `def`.

Inoltre, le variabili possono essere definite in tre diverse posizioni, corrispondenti a tre diversi `scopes`:
- se assegnata dentro un `def` è locale alla funzione,
- se assegnata racchiusa fra `def` è `nonlocal` rispetto alle `nested functions`,
- se è definite fuori da tutti i `def`s è `global`.

### Scope and Nested functions
#### Factory functions
`Factory functions` è riferito a funzioni composte che ricordano i valori dentro i propri `scopes` perché ancora presenti in memoria.

Sebbene le classi sono state create apposya per esplicitare l'attributo assegnato, le `factory functions` sono un'alternativa in quanto sono spesso utilizzate per creare gestori di eventi al volo in risposta a condizioni a runtime.
...
Questa tecnica è simile a ciò che è possibile fare in linguaggi funzionali e spesso è più facile utilizzare `lambda` nested in `def`.

#### Retaining enclosing scopes’ state with defaults
È possibile definire valori di default per argomenti:
```python
def name(x = 10): ...
```
oppure assegnarli dallo `scope`
```python
x = ...
def name(x=x): ...
```
...
Non come per `global`, un `nonlocal name` deve esistestere nella `enclosing function's scope` per poter essere utilizzato. Nell'insieme ha un comportamento simile a quello di `global`.
...
## Arguments
### Arguments-Passing Basic
Gli argomenti sono passati per `assignment`:
- **Arguments are passed by automatically assigning objects to local variable
names.** Gli argomenti sono (quando è possibile) riferimenti a `shared objects` perché i riferimenti sono implementati come puntatori.
- **Assigning to argument names inside a function does not affect the caller.**
- **Changing a mutable object argument in a function may impact the caller.**
- **Immutable arguments (e.g. int) are effectively passed “by value.”**
- **Mutable arguments are effectively passed “by pointer.”**

Per evitare che un `object` mutabile modificato abbia effetti sul chiamante è necessario passare alla `function` invocata un nuovo `objects`:
```python
L = [1, 2]
changer(X, L[:])
...
changer(X, tuple(L))
```
### Special Arguments-Matching Modes
Gli argomenti sono passati per `assignment`. Tuttavia Python fornisce strumenti (opzionali) aggiuntivi che alterano il modo in cui gli oggetti degli argomenti in una chiamata vengono abbinati ai nomi degli argomenti nell'intestazione prima dell'assegnazione.

#### The Basics
Il meccanismo di passaggio dopo che il `matching` ha luogo è ancora l'assegnazione.
